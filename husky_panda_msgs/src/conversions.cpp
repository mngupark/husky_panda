...
#include "husky_panda_msgs/conversions.h"
...
void msgToEigen(const husky_panda_msgs::State &stateRos,
  ...
  state.resize(husky_panda_msgs::State::SIZE);
...
void eigenToMsg(const Eigen::VectorXd &state, const double &time,
                husky_panda_msgs::State &stateRos) {
...
void msgToEigen(const husky_panda_msgs::Input &inputRos,
  ...
  input = Eigen::VectorXd::Zero(husky_panda_msgs::Input::SIZE);
...
void fromEigenState(Eigen::Vector3d &base_pose,
  ...
  assert(state.size() == husky_panda_msgs::State::SIZE);
  base_pose.x() = state(0);
  base_pose.y() = state(1);
  base_pose.z() = state(2);
