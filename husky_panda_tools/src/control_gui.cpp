...
static void resize_callback(GLFWwindow* window, int width, int height) {
  auto g =
      static_cast<husky_panda_tools::ControlGui*>(glfwGetWindowUserPointer(window));
...
struct RolloutData {
  const husky_panda_control::Rollout* rollout;
...
void ControlGui::reset_config(const husky_panda_control::config_t& config) {
  config_ = config;
}

void ControlGui::reset_averaged_policy(const input_array_t& u) { uu_avg_ = u; }

void ControlGui::reset_input(const husky_panda_control::input_t& u, const double t) {
