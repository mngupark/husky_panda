...
//  husky_panda_tools::VisualDebugger gui{};
...
int main(int argc, char** argv) {
  ...
  husky_panda_tools::ControlGui gui;
  gui.init();

  size_t n = 20;
  double counter = 0;

  husky_panda_control::Config config;
  config.rollouts = 10;
  ...
  std::vector<husky_panda_control::Rollout> rollouts(
      config.rollouts, husky_panda_control::Rollout(steps, input_dim, state_dim));
  husky_panda_control::Rollout averaged(steps, input_dim, state_dim);
  husky_panda_control::Rollout filtered(steps, input_dim, state_dim);

  std::vector<double> variance{0.01, 0.01, 0.5, 1.0};
  husky_panda_control::GaussianSampler sampler(input_dim);
