...
#include "husky_panda_ros/conversions.h"
#include "husky_panda_ros/ros_params.h"

using namespace husky_panda_ros;
...
bool ControllerRos::init_default_params() {
  ...
  ok &=
      husky_panda_ros::getNonNegative(nh_, "policy_update_rate", policy_update_rate_);
  ok &= husky_panda_ros::getNonNegative(nh_, "reference_update_rate",
                                 reference_update_rate_);
  ok &= husky_panda_ros::getNonNegative(nh_, "ros_publish_rate", ros_publish_rate_);
  ok &= husky_panda_ros::getBool(nh_, "publish_ros", publish_ros_);
...
void ControllerRos::init_default_ros() {
  ...
  data_publisher_ = nh_.advertise<husky_panda_ros::Data>("/husky_panda_control_data", 10);
...
bool ControllerRos::start() {
  ...
  husky_panda_control::threading::WorkerOptions update_policy_opt;
  ...
  husky_panda_control::threading::WorkerOptions update_reference_opt;
  ...
  if (publish_ros_) {
    husky_panda_control::threading::WorkerOptions publish_ros_opt;
...
bool ControllerRos::update_policy() {
  ...
  if (controller_->config_.logging) {
    husky_panda_ros::to_msg(controller_->get_data(), data_ros_);
...
bool ControllerRos::update_reference_thread(
    const husky_panda_control::threading::WorkerEvent &event) {
...
void ControllerRos::publish_input() {
  husky_panda_control::input_t input_copy_;
  {
...
void ControllerRos::set_observation(const husky_panda_control::observation_t &x,
                                    const double &t) {
  controller_->set_observation(x, t);
  observation_set_ = true;
}

void ControllerRos::get_input(const husky_panda_control::observation_t &x, husky_panda_control::input_t &u,
                              const double &t) {
