...
class Plotter:
    def __init__(self, experiment_id="husky_panda_control_panda"):
        self.df = self.get_data(experiment_id)

    def get_data(self, experiment_id):
        csv_file = os.path.join(RosPack().get_path("husky_panda_ros"), "log",
                                "record.csv")
    ...
    def plot_rate(self):
        ...
        for experiment_id, color in zip(self.df['id'].unique(), colors):
            ...
            ax.plot(time_average_rate,
                    average_rate,
                    linewidth=2,
                    label=experiment_id.strip("husky_panda_control"),
                    c=color)
            ax.set_yticklabels(ax.get_yticks(), size=30)
