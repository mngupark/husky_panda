...
#include "husky_panda_control/tree_search/experts/expert.h"
#include "husky_panda_control/tree_search/tree/node.h"
#include "husky_panda_control/tree_search/tree/tree.h"

#include "husky_panda_control/utils/data_logger.h"
#include "husky_panda_control/utils/gaussian_sampler.h"
#include "husky_panda_control/utils/thread_pool.h"
...
class TreeManager {
 public:
  ...
  TreeManager(const dynamics_ptr& dynamics, cost_ptr cost, sampler_ptr sampler,
              config_t config, husky_panda_control::Expert* expert);
  ~TreeManager() = default;
 ...
 public:
  ...
  void build_new_tree(const observation_t& x0_internal, double t0_internal,
                      const husky_panda_control::Rollout& opt_roll);

  /**
   * @brief Returns the rollouts sampled by the tree
   * @return vector of rollouts sampled by the tree
   */
  std::vector<husky_panda_control::Rollout> get_rollouts();
  ...
  void set_reference_trajectory(const husky_panda_control::reference_trajectory_t& traj);
 ...
 private:
  ...
  /**
   * @brief Transforms the tree into the rollouts structure used by husky_panda_control
   */
 ...
 private:
  ...
  husky_panda_control::Rollout opt_roll_;     // copy of optimal rollout from PathIntegral
  ...
  std::vector<husky_panda_control::Rollout> rollouts_;  // internal tree rollouts
