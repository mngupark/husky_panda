...
#include "husky_panda_control/tree_search/tree/tree_manager.h"
// #include <omp.h>
...
TreeManager::TreeManager(const dynamics_ptr& dynamics, cost_ptr cost,
                         ...
                         husky_panda_control::Expert* expert)
    : sampling_tree_() {
  cost_ = std::move(cost);
  dynamics_ = dynamics;
  config_ = std::move(config);
  sampler_ = std::move(sampler);
  expert_ = static_cast<std::shared_ptr<husky_panda_control::Expert>>(expert);
  ...
  rollouts_.resize(
      config_.rollouts,
      husky_panda_control::Rollout(tree_target_depth_, dynamics_->get_input_dimension(),
...
void TreeManager::set_reference_trajectory(
    const husky_panda_control::reference_trajectory_t& traj) {
...
std::vector<husky_panda_control::Rollout> TreeManager::get_rollouts() { return rollouts_; }
