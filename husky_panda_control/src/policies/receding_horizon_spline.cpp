...
#include "husky_panda_control/utils/utils.h"

#include <cassert>

using namespace husky_panda_control;

Eigen::ArrayXd husky_panda_control::B(const Eigen::ArrayXd &x, int k, int i,
  ...
  if (t(i + k) == t(i))
    ...
    c1 = (1 / (t(i + k) - t(i))) *
         (x - t(i)).cwiseProduct(husky_panda_control::B(x, k - 1, i, t));

  if (t(i + k + 1) == t(i + 1))
    c2.setZero();
  else
    c2 = (1 / (t(i + k + 1) - t(i + 1))) *
         (t(i + k + 1) - x).cwiseProduct(husky_panda_control::B(x, k - 1, i + 1, t));
  return c1 + c2;
}

Eigen::ArrayXd husky_panda_control::B2(const Eigen::ArrayXd &x, int k, int i,
                          const Eigen::ArrayXd &t) {
  assert(t.size() >= k + 1 &&
         "Base spline of order k requires at least support of size k+1");
