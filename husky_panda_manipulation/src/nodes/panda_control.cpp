...
int main(int argc, char** argv) {
  ...
  observation_t x_nom;
  husky_panda_msgs::State x_nom_ros;
  ros::Publisher x_nom_publisher_ =
      nh.advertise<husky_panda_msgs::State>("/observer/state", 10);

  // init state and input
  husky_panda_control::observation_t x = simulation->get_state();
  husky_panda_control::input_t u = simulation->get_zero_input(x);
  ...
  while (ros::ok()) {
    ...
    husky_panda_control::conversions::eigenToMsg(x_nom, sim_time, x_nom_ros);
