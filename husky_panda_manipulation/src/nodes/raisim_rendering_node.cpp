...
int main(int argc, char** argv) {
  ...
  husky_panda_control::PandaRaisimDynamics dynamics(
  ...
  husky_panda_control::DynamicsBase::observation_t current_state;

  auto cb = [&](const husky_panda_msgs::StateConstPtr& msg) {
    husky_panda_control::conversions::msgToEigen(*msg, current_state);
    dynamics.reset(current_state, 0);
  };

  ros::Subscriber current_state_subscriber =
      nh.subscribe<husky_panda_msgs::State>("/observer/state", 1, cb);
